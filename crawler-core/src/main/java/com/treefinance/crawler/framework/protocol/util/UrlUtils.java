...
public class UrlUtils {
    ...
    public static String extractUrlContent(String url) {
        return PatternUtils.group(url, URL_NAME_PATTERN, 5);
    }
    ...
    public static String fixUrl(String url) {
        ...
        try {
            ...
            logger.info("invalid syntax with url : " + url);
        }
    ...
    /**
     * <p>
     * Encodes illegal characters in the specified URL's path, query string and anchor according to the URL encoding
     * rules observed in real browsers.
     * </p>
     * <p>
     * For example, this method changes <tt>"http://first/?a=b c"</tt> to <tt>"http://first/?a=b%20c"</tt>.
     * </p>
     * 
     ...
     * @exception MalformedURLException if there is a problem creating the new URL
     */
    private static URL createNewUrl(final String protocol, final String host, final int port, final String path, final String ref, final String query)
     ...
     * <a href="http://www.faqs.org/rfcs/rfc1808.html">RFC1808</a>: Section 2.4: Parsing a URL An accepted method for
     * parsing URLs is useful to clarify the generic-RL syntax of Section 2.2 and to describe the algorithm for
     * resolving relative URLs presented in Section 4. This section describes the parsing rules for breaking down a URL
     * (relative or absolute) into the component parts described in Section 2.1. The rules assume that the URL has
     * already been separated from any surrounding text and copied to a "parse string". The rules are listed in the
    ...
    /**
     * Resolves a given relative URL against a base URL using the algorithm depicted in
     * <a href="http://www.faqs.org/rfcs/rfc1808.html">RFC1808</a>: Section 4: Resolving Relative URLs This section
     * describes an example algorithm for resolving URLs within a context in which the URLs may be relative, such that
     * the result is always a URL in absolute form. Although this algorithm cannot guarantee that the resulting URL will
     * equal that intended by the original author, it does guarantee that any valid URL (relative or absolute) can be
    ...
    public static void main(String[] args) {
        System.out.println(UrlUtils.checkUrlSyntax("dhttp://web.kugou.com/?hash=22f7961e122079607023be47adcb3821&microblog=1&filename=cnblue__-__love"));
    }
