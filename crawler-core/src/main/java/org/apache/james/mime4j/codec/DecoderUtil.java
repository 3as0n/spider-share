...
public class DecoderUtil {
    ...
    private static String tryDecodeEncodedWord(final String mimeCharset, final String encoding, final String encodedText) {
        ...
        if (encodedText.length() == 0) {
            if (log.isWarnEnabled()) {
                log.warn("Missing encoded text in encoded word: '" + recombine(mimeCharset, encoding, encodedText) + "'");
            }
            return null;
        }
        ...
        try {
            if (encoding.equalsIgnoreCase("Q")) {
                ...
                if (log.isWarnEnabled()) {
                    log.warn("Warning: Unknown encoding in encoded word '" + recombine(mimeCharset, encoding, encodedText) + "'");
                }
                return null;
            }
            ...
            if (log.isWarnEnabled()) {
                log.warn("Unsupported encoding in encoded word '" + recombine(mimeCharset, encoding, encodedText) + "'", e);
            }
            return null;
        } catch (RuntimeException e) {
            if (log.isWarnEnabled()) {
                log.warn("Could not decode encoded word '" + recombine(mimeCharset, encoding, encodedText) + "'", e);
            }
            return null;
        }
    }
