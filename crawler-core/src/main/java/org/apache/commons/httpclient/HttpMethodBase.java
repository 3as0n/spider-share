 ...
 * <ul>
 * <li>{@link #addRequestHeaders addRequestHeaders(HttpState,HttpConnection)} to write those headers</li>
 * </ul>
 * </p>
 * <p>
 * When a method expects specific response headers, subclasses may want to override:
 * <ul>
 * <li>{@link #processResponseHeaders processResponseHeaders(HttpState,HttpConnection)} to handle those headers</li>
 * </ul>
 * </p>
 * 
...
public abstract class HttpMethodBase implements HttpMethod {
    ...
    /** cookie reset regex suffix. */
    private final String COOKIE_RESET_REGEX_SUFFIX = PropertiesConfiguration.getInstance().get("cookie.reset.regex.suffix", "=(((?!; ).)*)");
    ...
    public HttpMethodBase() {
        LOG.info("Using custom http method");
    }

    /**
     * Constructor specifying a URI. It is responsibility of the caller to ensure that URI elements (path & query
     * parameters) are properly encoded (URL safe).
     * 
     * @param uri either an absolute or relative URI. The URI is expected to be URL-encoded
     * @exception IllegalArgumentException when URI is invalid
     * @exception IllegalStateException when protocol of the absolute URI is not recognised
     */
    public HttpMethodBase(String uri) throws IllegalArgumentException, IllegalStateException {
        LOG.info("Using custom http method: " + uri);
        try {
     ...
     * @return <tt>true</tt> if the message may contain a body, <tt>false</tt> if it can not contain a message body
     */
    ...
    /**
     * Obtains the name of the HTTP method as used in the HTTP request line, for example <tt>"GET"</tt> or
     * <tt>"POST"</tt>.
     * 
     ...
     * @exception URIException if a URI cannot be set
     * @since 3.0
     */
    @Override
    ...
    /**
     * Returns <tt>true</tt> if the HTTP method should automatically follow HTTP redirects (status code 302, etc.),
     * <tt>false</tt> otherwise.
     * 
     * @return <tt>true</tt> if the method will automatically follow HTTP redirects, <tt>false</tt> otherwise.
     */
    @Override
     ...
     * 
     * @param followRedirects <tt>true</tt> if the method will automatically follow redirects, <tt>false</tt> otherwise.
     */
    @Override
    ...
    /**
     * Returns <tt>true</tt> if the HTTP method should automatically handle HTTP authentication challenges (status code
     * 401, etc.), <tt>false</tt> otherwise
     * 
    ...
    /**
     * Sets whether or not the HTTP method should automatically handle HTTP authentication challenges (status code 401,
     * etc.)
     * 
     * @param doAuthentication <tt>true</tt> to process authentication challenges authomatically, <tt>false</tt>
     *        otherwise.
     * @since 2.0
     */
    @Override
    ...
    /**
     * Returns <tt>true</tt> if version 1.1 of the HTTP protocol should be used per default, <tt>false</tt> if version
     * 1.0 should be used.
     * 
    ...
    /**
     * Adds the specified request header, NOT overwriting any previous value. Note that header-name matching is case
     * insensitive.
     * 
    ...
    /**
     * Gets the path of this HTTP method. Calling this method <em>after</em> the request has been executed will return
     * the <em>actual</em> path, following any redirects automatically handled by this HTTP method.
     * 
    ...
    /**
     * Sets the path of the HTTP method. It is responsibility of the caller to ensure that the path is properly encoded
     * (URL safe).
     * 
    ...
    /**
     * Sets the query string of this HTTP method. The caller must ensure that the string is properly URL encoded. The
     * query string should not start with the question mark character.
     * 
    ...
    /**
     * Sets the query string of this HTTP method. The pairs are encoded as UTF-8 characters. To use a different charset
     * the parameters can be encoded manually using EncodingUtil and set as a single String.
     * 
     * @param params an array of {@link NameValuePair}s to add as query string parameters. The name/value pairs will be
     *        automcatically URL encoded
     * @see EncodingUtil#formUrlEncode(NameValuePair[], String)
    ...
    /**
     * Set the specified request header, overwriting any previous value. Note that header-name matching is
     * case-insensitive.
     * 
    ...
    /**
     * Sets the specified request header, overwriting any previous value. Note that header-name matching is case
     * insensitive.
     * 
    ...
    /**
     * Returns the specified request header. Note that header-name matching is case insensitive. <tt>null</tt> will be
     * returned if either <i>headerName</i> is <tt>null</tt> or there is no matching header for <i>headerName</i>.
     *
    ...
    /**
     * Gets the response header associated with the given name. Header name matching is case insensitive. <tt>null</tt>
     * will be returned if either <i>headerName</i> is <tt>null</tt> or there is no matching header for
     * <i>headerName</i>.
     *
     ...
     * </p>
     *
     * @return content length, if <tt>Content-Length</tt> header is available. <tt>0</tt> indicates that the request has
     *         no body. If <tt>Content-Length</tt> header is not present, the method returns <tt>-1</tt>.
     */
    ...
    /**
     * Returns the response body of the HTTP method, if any, as an array of bytes. If response body is not available or
     * cannot be read, returns <tt>null</tt>. Buffers the response and this method can be called several times yielding
     * the same result each time. Note: This will cause the entire response body to be buffered in memory. A malicious
     * server may easily exhaust all the VM memory. It is strongly recommended, to use getResponseAsStream if the
     * content length of the response is unknown or resonably large.
     *
     * @return The response body.
     * @exception IOException If an I/O (transport) problem occurs while obtaining the response body.
     */
    @Override
     ...
     * Returns the response body of the HTTP method, if any, as an array of bytes. If response body is not available or
     * cannot be read, returns <tt>null</tt>. Buffers the response and this method can be called several times yielding
     * the same result each time. Note: This will cause the entire response body to be buffered in memory. This method
     * is safe if the content length of the response is unknown, because the amount of memory used is limited.
     * <p>
     * If the response is large this method involves lots of array copying and many object allocations, which makes it
     * unsuitable for high-performance / low-footprint applications. Those applications should use
    ...
    public byte[] getResponseBody(int maxlen) throws IOException {
        ...
        if (this.responseBody == null) {
            ...
            if (instream != null) {
                ...
                LOG.debug("Buffering response body");
                ByteArrayOutputStream rawdata = new ByteArrayOutputStream(contentLength > 0 ? (int)contentLength : DEFAULT_INITIAL_BUFFER_SIZE);
    ...
    /**
     * Returns the response body of the HTTP method, if any, as an {@link InputStream}. If response body is not
     * available, returns <tt>null</tt>. If the response has been buffered this method returns a new stream object on
     * every call. If the response has not been buffered the returned stream can only be read once.
     *
    ...
    /**
     * Returns the response body of the HTTP method, if any, as a {@link String}. If response body is not available or
     * cannot be read, returns <tt>null</tt> The string conversion on the data is done using the character encoding
     * specified in <tt>Content-Type</tt> header. Buffers the response and this method can be called several times
     * yielding the same result each time. Note: This will cause the entire response body to be buffered in memory. A
     * malicious server may easily exhaust all the VM memory. It is strongly recommended, to use getResponseAsStream if
     ...
     * cannot be read, returns <tt>null</tt> The string conversion on the data is done using the character encoding
     * specified in <tt>Content-Type</tt> header. Buffers the response and this method can be called several times
     * yielding the same result each time.
     * </p>
     ...
     * {@link #getResponseBodyAsStream()}.
     *
     * @param maxlen the maximum content length to accept (number of bytes). Note that, depending on the encoding, this
     *        is not equal to the number of characters.
     * @return The response body or <code>null</code>.
     * @exception IOException If an I/O (transport) problem occurs while obtaining the response body.
     */
    ...
    /**
     * Gets the response footer associated with the given name. Footer name matching is case insensitive. <tt>null</tt>
     * will be returned if either <i>footerName</i> is <tt>null</tt> or there is no matching footer for
     * <i>footerName</i> or there are no footers available. If there are multiple footers with the same name, there
     * values will be combined with the ',' separator as specified by RFC2616.
     *
    ...
    /**
     * @return <tt>false</tt>
     * @deprecated Use {@link org.apache.commons.httpclient.params.HttpParams#setParameter(String, Object)} to exercise
    ...
    /**
     * Defines how strictly HttpClient follows the HTTP protocol specification (RFC 2616 and other relevant RFCs). In
     * the strict mode HttpClient precisely implements the requirements of the specification, whereas in non-strict mode
     * it attempts to mimic the exact behaviour of commonly used HTTP agents, which many HTTP servers expect.
     *
     ...
     *
     * @param state {@link HttpState state} information to associate with this request. Must be non-null.
     * @param conn the {@link HttpConnection connection} to used to execute this HTTP method. Must be non-null.
     * @return the integer status code if one was obtained, or <tt>-1</tt>
     * @exception IOException if an I/O (transport) error occurs
     * @exception HttpException if a protocol exception occurs.
     */
    @Override
    ...
    /**
     * Returns <tt>true</tt> if the HTTP method has been already {@link #execute executed}, but not {@link #recycle
     * recycled}.
     *
    ...
    /**
     * Recycles the HTTP method so that it can be used again. Note that all of the instance variables will be reset once
     * this method has been called. This method will also release the connection being used by this HTTP method.
     *
    ...
    /**
     * Releases the connection being used by this HTTP method. In particular the connection is used to read the
     * response(if there is one) and will be held until the response has been read. If the connection can be reused by
    ...
    /**
     * Returns the HTTP version used with this method (may be <tt>null</tt> if undefined, that is, the method has not
     * been executed)
     *
     * @return HTTP version.
     * @since 3.0
    ...
    /**
     * Returns proxy authentication realm, if it has been used during authentication process. Otherwise returns
     * <tt>null</tt>.
     *
    ...
    /**
     * Returns authentication realm, if it has been used during authentication process. Otherwise returns <tt>null</tt>.
     *
     ...
     * @deprecated no longer used Returns the number of "recoverable" exceptions thrown and handled, to allow for
    ...
    /**
     * Returns <tt>true</tt> if the HTTP has been transmitted to the target server in its entirety, <tt>false</tt>
     * otherwise. This flag can be useful for recovery logic. If the request has not been transmitted in its entirety,
    ...
    /**
     * Gets the {@link HeaderGroup header group} storing the response trailer headers as per RFC 2616 section 3.6.1.
     *
     * @return a HeaderGroup
     * @since 2.0beta1
     */
    ...
    /**
     * Returns a stream from which the body of the current response may be read. If the method has not yet been
     * executed, if <code>responseBodyConsumed</code> has been called, or if the stream returned by a previous call has
     * been closed, <code>null</code> will be returned.
     *
    ...
    /**
     * Sets whether or not the connection should be force-closed when no longer needed. This value should only be set to
     * <code>true</code> in abnormal circumstances, such as HTTP protocol violations.
     *
    ...
    /**
     * Tests if the connection should be closed after the method has been executed. The connection will be left open
     * when using HTTP/1.1 or if <tt>Connection:
    ...
    protected void addDistinctCookieRequestHeader(HttpState state, HttpConnection conn) throws IOException, HttpException {
        LOG.trace("enter HttpMethodBase.addCookieRequestHeader(HttpState, " + "HttpConnection)");
        boolean retainQuote = BooleanUtils.isFalse(((CustomGetMethod)this).isCoexist());
        ...
        if ((cookies != null) && (cookies.length > 0)) {
            ...
            for (Header cookieheader : cookieheaders) {
                ...
                // continue;
                // }
                cookieMap.putAll(CookiesFormatter.parseAsMap(cookieheader.getValue(), retainQuote));
            }
            String s = matcher.formatCookies(cookies);
            Map<String, String> newMap = CookiesFormatter.parseAsMap(s, retainQuote);
            cookieMap.putAll(newMap);
            if (getParams().isParameterTrue(HttpMethodParams.SINGLE_COOKIE_HEADER)) {
                // In strict mode put all cookies on the same header
                getRequestHeaderGroup().addHeader(new Header("Cookie", CookiesFormatter.toCookiesString(cookieMap), true));
            } else {
                // In non-strict mode put each cookie on a separate header
                String[] cookieArray = CookiesFormatter.toCookieArray(cookieMap, retainQuote);
                for (String value : cookieArray) {
                    getRequestHeaderGroup().addHeader(new Header("Cookie", value, true));
                }
            }
        }
    }
    ...
    /**
     * Generates <tt>Cookie</tt> request headers for those {@link Cookie cookie}s that match the given host, port and
     * path.
     *
    ...
    protected void addCookieRequestHeader(HttpState state, HttpConnection conn) throws IOException, HttpException {
        ...
        LOG.trace("enter HttpMethodBase.addCookieRequestHeader(HttpState, " + "HttpConnection)");
        // custom code
        if (this instanceof CustomGetMethod && BooleanUtils.isFalse(((CustomGetMethod)this).isCoexist())) {
            addDistinctCookieRequestHeader(state, conn);
            return;
        }
    ...
    /**
     * Generates <tt>Host</tt> request header, as long as no <tt>Host</tt> request header already exists.
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn the {@link HttpConnection connection} used to execute this HTTP method
     * @exception IOException if an I/O (transport) error occurs. Some transport exceptions can be recovered from.
    ...
    /**
     * Generates all the required request {@link Header header}s to be submitted via the given {@link HttpConnection
     * connection}.
     * <p>
     * This implementation adds <tt>User-Agent</tt>, <tt>Host</tt>, <tt>Cookie</tt>, <tt>Authorization</tt>,
     * <tt>Proxy-Authorization</tt> and <tt>Proxy-Connection</tt> headers, when appropriate.
     * </p>
     * <p>
     * Subclasses may want to override this method to to add additional headers, and may choose to invoke this
     * implementation (via <tt>super</tt>) to add the "standard" headers.
     * </p>
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn the {@link HttpConnection connection} used to execute this HTTP method
    ...
    /**
     * Generates default <tt>User-Agent</tt> request header, as long as no <tt>User-Agent</tt> request header already
     * exists.
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn the {@link HttpConnection connection} used to execute this HTTP method
    ...
    /**
     * Throws an {@link IllegalStateException} if the HTTP method has been already {@link #execute executed}, but not
     * {@link #recycle recycled}.
     *
     * @exception IllegalStateException if the method has been used and not recycled
     */
    ...
    /**
     * Throws an {@link IllegalStateException} if the HTTP method has not been {@link #execute executed} since last
     * {@link #recycle recycle}.
     *
     * @exception IllegalStateException if not used
     */
    ...
    /**
     * This method is invoked immediately after {@link #readResponseBody(HttpState, HttpConnection)} and can be
     * overridden by sub-classes in order to provide custom body processing.
     * <p>
    ...
    /**
     * This method is invoked immediately after {@link #readResponseHeaders(HttpState, HttpConnection)} and can be
     * overridden by sub-classes in order to provide custom response headers processing.
     * <p>
     * This implementation will handle the <tt>Set-Cookie</tt> and <tt>Set-Cookie2</tt> headers, if any, adding the
            ...
            if (cookies != null) {
                for (int j = 0; j < cookies.length; j++) {
                    ...
                    try {
                        ...
                        // custom code
                        if (state instanceof CustomHttpState) {
                            // resrt cookie value
                            String originalValue = cookie.getValue();
                            String value = PatternUtils.group(header.toString(), cookie.getName() + COOKIE_RESET_REGEX_SUFFIX, 1);
                            if (StringUtils.isNotBlank(value) && !value.equals(originalValue)) {
                                LOG.warn("Reset cookie: " + cookie.getName() + "'value from: " + originalValue + " to: " + value);
    ...
    /**
     * This method is invoked immediately after {@link #readStatusLine(HttpState, HttpConnection)} and can be overridden
     * by sub-classes in order to provide custom response status line processing.
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn the {@link HttpConnection connection} used to execute this HTTP method
     * @see #readResponse
     ...
     * <li>{@link #processStatusLine(HttpState, HttpConnection)} is invoked, allowing the method to process the status
     * line if desired.</li>
     * <li>{@link #readResponseHeaders(HttpState, HttpConnection)} is invoked to read the associated headers.</li>
     * <li>{@link #processResponseHeaders(HttpState, HttpConnection)} is invoked, allowing the method to process the
     * headers if desired.</li>
     * <li>{@link #readResponseBody(HttpState, HttpConnection)} is invoked to read the associated body (if any).</li>
     * <li>{@link #processResponseBody(HttpState, HttpConnection)} is invoked, allowing the method to process the
     * response body if desired.</li>
     * </ol>
     * Subclasses may want to override one or more of the above methods to to customize the processing. (Or they may
     * choose to override this method if dramatically different processing is required.)
     * </p>
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn the {@link HttpConnection connection} used to execute this HTTP method
     ...
     * <p>
     * The current implementation wraps the socket level stream with an appropriate stream for the type of response
     * (chunked, content-length, or auto-close). If there is no response body, the connection associated with the
     * request will be returned to the connection manager.
     * </p>
     * <p>
     ...
     * changing the semantics of the message, by appending each subsequent field-value to the first, each separated by a
     * comma." - HTTP/1.0 (4.3)
     * </p>
     *
     * @param state the {@link HttpState state} information associated with this method
    ...
    protected void readResponseHeaders(HttpState state, HttpConnection conn) throws IOException, HttpException {
        ...
        if (LOG.isDebugEnabled() && ArrayUtils.isNotEmpty(headers)) {
    ...
    /**
     * Read the status line from the given {@link HttpConnection}, setting my {@link #getStatusCode status code} and
     ...
     * </p>
     * <ol>
     * <li>{@link #writeRequestLine(HttpState, HttpConnection)} is invoked to write the request line.</li>
     * <li>{@link #writeRequestHeaders(HttpState, HttpConnection)} is invoked to write the associated headers.</li>
     * <li><tt>\r\n</tt> is sent to close the head part of the request.</li>
     * <li>{@link #writeRequestBody(HttpState, HttpConnection)} is invoked to write the body part of the request.</li>
     * </ol>
     * <p>
     * Subclasses may want to override one or more of the above methods to to customize the processing. (Or they may
     * choose to override this method if dramatically different processing is required.)
     ...
     * <p>
     * This method should return <tt>true</tt> if the request body was actually sent (or is empty), or <tt>false</tt> if
     * it could not be sent for some reason.
     * </p>
     * <p>
     ...
     * <p>
     * This implementation invokes {@link #addRequestHeaders(HttpState, HttpConnection)}, and then writes each header to
     * the request stream.
     * </p>
     * <p>
     * Subclasses may want to override this method to to customize the processing.
     ...
     * <p>
     * The default behavior for this class is to check to see if the connection should be closed, and close if need be,
     * and to ensure that the connection is returned to the connection manager - if and only if we are not still inside
     * the execute call.
     * </p>
     */
    ...
    /**
     * This method is a dirty hack intended to work around current (2.0) design flaw that prevents the user from
     * obtaining correct status code, headers and response body from the preceding HTTP CONNECT method. TODO: Remove
     * this crap as soon as possible
     */
     ...
     * @exception HttpException If the method is in invalid state.
     */
    ...
    /**
     * Returns the response body as an {@link InputStream input stream} corresponding to the values of the
     * <tt>Content-Length</tt> and <tt>Transfer-Encoding</tt> headers. If no response body is available returns
     * <tt>null</tt>.
     * <p>
     *
     * @param conn the {@link HttpConnection connection} used to execute this HTTP method
