...
/**
 * An HttpCookie object represents an HTTP cookie, which carries state information between server and user agent. Cookie
 * is widely adopted to create stateful sessions.
 *
 * <p>
 * There are 3 HTTP cookie specifications: <blockquote> Netscape draft<br>
 * RFC 2109 - <a href="http://www.ietf.org/rfc/rfc2109.txt"> <i>http://www.ietf.org/rfc/rfc2109.txt</i></a><br>
 * RFC 2965 - <a href="http://www.ietf.org/rfc/rfc2965.txt"> <i>http://www.ietf.org/rfc/rfc2965.txt</i></a>
 * </blockquote>
 *
 * <p>
...
public final class HttpCookie implements Cloneable {
    ...
    private final static String[] COOKIE_DATE_FORMATS = {"EEE',' dd-MMM-yyyy HH:mm:ss 'GMT'", "EEE',' dd MMM yyyy HH:mm:ss 'GMT'", "EEE MMM dd yyyy HH:mm:ss 'GMT'Z",
        "EEE',' dd-MMM-yy HH:mm:ss 'GMT'", "EEE',' dd MMM yy HH:mm:ss 'GMT'", "EEE MMM dd yy HH:mm:ss 'GMT'Z"};
    ...
    static {
        ...
        assignors.put("expires", new HttpCookie.CookieAttributeAssignor() { // Netscape only
            @Override
            public void assign(HttpCookie cookie, String attrName, String attrValue) {
                if (cookie.getMaxAge() == MAX_AGE_UNSPECIFIED) {
     ...
     *
     * <p>
     * The name must conform to RFC 2965. That means it can contain only ASCII alphanumeric characters and cannot
     ...
     * cookie's value can be changed after creation with the {@code setValue} method.
     *
     * <p>
     * By default, cookies are created according to the RFC 2965 cookie specification. The version can be changed with
     * the {@code setVersion} method.
     *
     *
    ...
    /**
     * Constructs cookies from set-cookie or set-cookie2 header string. RFC 2965 section 3.2.2 set-cookie2 syntax
     * indicates that one header line may contain more than one cookie definitions, so this is a static utility method
     * instead of another constructor.
     *
     * @param header a {@code String} specifying the set-cookie header. The header should start with "set-cookie", or
     *        "set-cookie2" token; or it should have no leading token at all.
     *
     * @return a List of cookie parsed from header line string
     *
     * @throws IllegalArgumentException if header string violates the cookie specification's syntax or the cookie name
    ...
    public static List<HttpCookie> parse(String header, boolean retainQuote) {
        return parse(header, false, retainQuote);
    }
    ...
    // class.
    private static List<HttpCookie> parse(String header, boolean retainHeader, boolean retainQuote) {
        ...
        if (version == 0) {
            ...
            HttpCookie cookie = parseInternal(header, retainHeader, retainQuote);
            cookie.setVersion(0);
            cookies.add(cookie);
        } else {
            // rfc2965/2109 cookie
            ...
            for (String cookieStr : cookieStrings) {
                HttpCookie cookie = parseInternal(cookieStr, retainHeader, retainQuote);
                cookie.setVersion(1);
                cookies.add(cookie);
            }
        }
     ...
     *
     * <p>
     * This concept is described in the cookie specification. To understand the concept, some terminologies need to be
     * defined first: <blockquote> effective host name = hostname if host name contains dot<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;or =
     * hostname.local if not </blockquote>
     * <p>
     * Host A's name domain-matches host B's if: <blockquote>
     * <ul>
     * <li>their host name strings string-compare equal; or</li>
     * <li>A is a HDN string and has the form NB, where N is a non-empty name string, B has the form .B', and B' is a
     * HDN string. (So, x.y.com domain-matches .Y.com but not Y.com.)</li>
     * </ul>
     * </blockquote>
     *
     * <p>
     ...
     * <li>The request-host is a HDN (not IP address) and has the form HD, where D is the value of the Domain attribute,
     * and H is a string that contains one or more dots.</li>
     * </ul>
     * </blockquote>
     *
     * <p>
     * Examples: <blockquote>
     * <ul>
     * <li>A Set-Cookie2 from request-host y.x.foo.com for Domain=.foo.com would be rejected, because H is y.x and
     * contains a dot.</li>
     * <li>A Set-Cookie2 from request-host x.foo.com for Domain=.foo.com would be accepted.</li>
     * <li>A Set-Cookie2 with Domain=.com or Domain=.com., will always be rejected, because there is no embedded
     * dot.</li>
     * <li>A Set-Cookie2 from request-host example for Domain=.local will be accepted, because the effective host name
     * for the request- host is example.local, and example.local domain-matches .local.</li>
     * </ul>
     * </blockquote>
     *
     ...
     */
    private static HttpCookie parseInternal(String header, boolean retainHeader, boolean retainQuote) {
        ...
        try {
            ...
            if (index != -1) {
                ...
                if (!retainQuote) {
                    value = stripOffSurroundingQuote(value);
                }
                if (retainHeader) {
                    cookie = new HttpCookie(name, value, header);
                } else {
    ...
    /**
     * Specifies a comment URL that describes a cookie's purpose. The comment URL is useful if the browser presents the
     * cookie to the user. Comment URL is RFC 2965 only.
     *
    ...
    /**
     * Specify the portlist of the cookie, which restricts the port(s) to which a cookie may be sent back in a Cookie
     * header.
     *
     ...
     *
     * <p>
     * The form of the domain name is specified by RFC 2965. A domain name begins with a dot ({@code .foo.com}) and
     * means that the cookie is visible to servers in a specified Domain Name System (DNS) zone (for example,
     * {@code www.foo.com}, but not {@code a.b.foo.com}). By default, cookies are only returned to the server that sent
     * them.
     *
     * @param pattern a {@code String} containing the domain name within which this cookie is visible; form is according
     *        to RFC 2965
     *
     * @see #getDomain
     */
    ...
    /**
     * Returns the maximum age of the cookie, specified in seconds. By default, {@code -1} indicating the cookie will
     ...
     *
     * @param expiry an integer specifying the maximum age of the cookie in seconds; if zero, the cookie should be
     *        discarded immediately; otherwise, the cookie's max age is unspecified.
     *
     * @see #getMaxAge
     */
    ...
    /**
     * Returns {@code true} if sending this cookie should be restricted to a secure protocol, or {@code false} if the it
     * can be sent using any protocol.
     *
     * @return {@code false} if the cookie can be sent over any standard protocol; otherwise, {@code true}
     *
     * @see #setSecure
     */
     ...
     *
     * @param flag If {@code true}, the cookie can only be sent over a secure protocol like HTTPS. If {@code false}, it
    ...
    /**
     * Returns the version of the protocol this cookie complies with. Version 1 complies with RFC 2965/2109, and version
     * 0 complies with the original cookie specification drafted by Netscape. Cookies provided by a browser use and
     ...
     * cookie specification. Version 1 complies with RFC 2965/2109.
     *
    ...
    /**
     * Returns {@code true} if this cookie contains the <i>HttpOnly</i> attribute. This means that the cookie should not
     * be accessible to scripting engines, like javascript.
     *
    ...
    /**
     * Indicates whether the cookie should be considered HTTP Only. If set to {@code true} it means the cookie should
    ...
    /**
     * Constructs a cookie header string representation of this cookie, which is in the format defined by corresponding
     ...
     * (case-insensitive), and have same path (case-sensitive).
     *
    ...
    /**
     * Returns the hash code of this HTTP cookie. The result is the sum of hash code value of three significant
     * components of this cookie: name, domain, and path. That is, the hash code is the value of the expression:
